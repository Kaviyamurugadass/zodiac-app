...
const FEATURES = [
  { to: '/horoscope', title: 'Daily Horoscope', desc: 'Love, work, money & health', art: 'star', bg: 'from-violet-500/40 to-fuchsia-500/30' },
  { to: '/tarot', title: 'Tarot Reading', desc: 'Card of the day & spreads', art: 'tarot', bg: 'from-indigo-500/40 to-sky-500/30' },
    ...
    <div className="space-y-5">
      <header className="flex items-center justify-between pt-3">
        ...
        <Link to="/profile" className="glass flex flex-col items-center px-3 py-2" aria-label="Daily streak">
          <Art name="fire" className="size-9 animate-wiggle" />
          <span className="text-sm leading-none font-extrabold text-gold-300">{streak.count}</span>
        ...
        >
          <div className="rounded-[1.4rem] bg-night-900/85 p-4">
            <div className="flex items-center gap-3">
              <Art name={sign.id} className="size-20 animate-float" alt={sign.name} />
              <div className="flex-1">
                ...
                <p className="text-xs text-violet-200/80">
                  Mood: {today.mood.label} · Theme: {today.theme}
                </p>
              </div>
            </div>
            ...
            <div className="mt-3 flex flex-wrap gap-2 text-xs">
              {today.categories.map((c) => (
                <span key={c.key} className="flex items-center gap-1 rounded-full bg-white/8 py-0.5 pr-2.5 pl-1">
                  <Art name={c.art} className="size-6" alt={c.label} /> {c.rating}/5
                </span>
              ))}
            </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {FEATURES.map((f, i) => (
          ...
          >
            <Art name={f.art} className="size-14" />
